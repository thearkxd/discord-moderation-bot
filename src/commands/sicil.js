...
module.exports = {
  ...
  run: async (client, message, args, embed) => {
    ...
    if (data.length > 0) {
      data = data.map(x => `#${x.id} ${x.active ? emojis.mark : emojis.cross} **[${x.type}]** ${moment(x.date).format("LLL")} tarihinde, <@${x.staff}> tarafından, \`${x.reason}\` nedeniyle, ${x.type.toLowerCase().replace("-", " ")} cezası almış.`);
    ...
    } else return message.channel.send(embed.setDescription(`${member.toString()} üyesinin sicili temiz!`));
  },
};
